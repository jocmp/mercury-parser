...
describe('GeniusComExtractor', () => {
  describe('initial test case', () => {
    ...
    it('returns the title', async () => {
      ...
      // the article.
      assert.strictEqual(title, `Purple Rain`);
    });
    ...
    it('returns the author', async () => {
      ...
      // Update these values with the expected values from
      // the article.
      assert.strictEqual(author, 'Prince and the Revolution');
    });

    it('returns the date_published', async () => {
      const { date_published } = await result;

      // ISO date-only "1984-06-25" parsed as midnight UTC
      assert.strictEqual(date_published, '1984-06-25T00:00:00.000Z');
    });
