...
describe('RootExtractor', () => {
  it('only returns what the custom parser gives it if fallback is disabled', () => {
    ...
    const { url } = RootExtractor.extract(NYMagExtractor as any, {
      url: fullUrl,
      html,
      $,
      metaCache: [],
...
describe('cleanBySelectors($content, $, { clean })', () => {
  it('removes provided selectors from the content', () => {
    ...
    assert.strictEqual($content.find('.ad').length, 0);
    assert.strictEqual($content.find('.share').length, 0);
  });
});
      ...
      transforms: {
        ...
        noscript: ($node, $) => {
          ...
          if (
            ...
            ($children.get(0) as any).tagName.toLowerCase() === 'img'
          ) {
            return 'figure';
          }
...
describe('select(opts)', () => {
  ...
  it("returns a node's attr with an attr selector", () => {
    ...
    const result = select(opts);
    assert.strictEqual(result, '2016-09-07T09:07:59.000Z');
  });
  ...
  it('handles multiple matches when the content selector is an array', () => {
    ...
    const result = select(opts);
    assert.strictEqual($(result).find('img.lead-image').length, 1);
    assert.strictEqual($(result).find('.content-is-here').length, 1);
  });
